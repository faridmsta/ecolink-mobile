...
const Register = () => {
    ...
    const handleSubmit = () => {
        const nameError = nameValidator(userData.name.value);
        const surnameError = nameValidator(userData.surname.value);
        const emailError = emailValidator(userData.email.value);
        const passwordError = passwordValidator(userData.password.value);
    
        // Check if passwords match
        const passwordsMatch = userData.password.value === userData.passwordAgain.value;
    
        // Set errors if any
        setUserData({
            ...userData,
            name: { ...userData.name, error: nameError },
            surname: { ...userData.surname, error: surnameError },
            email: { ...userData.email, error: emailError },
            password: { ...userData.password, error: passwordError || (passwordsMatch ? '' : 'Passwords do not match') },
            passwordAgain: { ...userData.passwordAgain, error: passwordsMatch ? '' : 'Passwords do not match' },
        });
    
        // If there are no errors, redirect
        if (!nameError && !surnameError && !emailError && !passwordError && passwordsMatch) {
                    ...
                    <View style={{ gap: 10 }} >
                            ...
                            <View style={{ flex: 1 }} >
                                ...
                                <UserInput
                                    ...
                                    onChange={(e) => {
                                        setUserData({
                                            ...userData,
                                            name: { ...userData.name, value: e.nativeEvent.text }
                                        });
                                    }}
                                    errorText={userData.name.error}
                                />
                            </View>
                            <View style={{ flex: 1 }} >
                                <Text
                                ...
                                <UserInput
                                    ...
                                    onChange={(e) => {
                                        setUserData({
                                            ...userData,
                                            surname: { ...userData.surname, value: e.nativeEvent.text }
                                        });
                                    }}
                                    errorText={userData.surname.error}
                                />
                            </View>
                            ...
                            <View style={{ flex: 1 }} >
                                ...
                                <PhoneInput
                                    ...
                                    onChange={(e) => {
                                        setUserData({
                                            ...userData,
                                            phone: { ...userData.phone, value: e.nativeEvent.text }
                                        });
                                    }}
                                    errorText={userData.phone.error}
                                />
                            </View>
                        ...
                        <View style={{ flexDirection: 'row', gap: 10 }} >
                            ...
                            <View style={{ flex: 1 }} >
                                ...
                                <UserInput
                                    ...
                                    value={userData.passwordAgain.value}
                                    onChange={(e) => {
                                        setUserData({
                                            ...userData,
                                            passwordAgain: { ...userData.passwordAgain, value: e.nativeEvent.text }
                                        });
                                    }}
                                    errorText={userData.passwordAgain.error}
                                    secureTextEntry={true}
                                />
                            </View>
                        </View>
                        <View style={{ flexDirection: 'row', gap: 10 }} >
                          <Pressable onPress={()=>{router.push('/login');}} >
                            <Text style={{fontFamily:'poppins',color:Style.colors.greenLight}}  >Have an account?</Text>
                          </Pressable>
                        </View>
                        <View style={{ flexDirection: 'row', gap: 10 }} >
                            <View style={{ flex: 3 }} >
